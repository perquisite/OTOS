...
class Embedding(torch.nn.Module):
        def __init__(self, data_class, output_dim):
            super(Embedding, self).__init__()
            mid_num1 = 4096
            mid_num2 = 4096
            self.fc1 = nn.Linear(data_class, mid_num1)
            self.fc2 = nn.Linear(mid_num1, mid_num2)
            self.Embedding = nn.Linear(mid_num2, output_dim)
...
class IDCM_NN(nn.Module):
    """Network to learn text representations"""
    def __init__(self, img_input_dim=4096, text_input_dim=1024, output_dim =1024, num_class=10):
        super(IDCM_NN, self).__init__()
        self.img_net = ImgNN(img_input_dim, output_dim = output_dim)
        self.text_net = TextNN(text_input_dim, output_dim = output_dim)
    ...
    def forward(self, img, text):
        ...
        # view1_predict = F.softmax(self.classifier(view1_feature), dim=1)
        # view2_predict = F.softmax(self.classifier(view2_feature), dim=1)
